...
__After 2025-12-21__ the transverse Mercator projections are based on the expansions of [Karney 2011](https://arxiv.org/abs/1002.1417), [Kawase 2011](http://www.gsi.go.jp/common/000062452.pdf), and [Kawase 2013](http://www.gsi.go.jp/common/000065826.pdf). The original DMATM implementation is also present for reference.
...
This will output a series of forward and backward conversions between latitude/longitude and the UTM grid, reproducing tables 2-11 and 3-7 of DMATM 8358.2, [which is included in this repository](https://github.com/sfegan/utm/blob/main/ADA266497.pdf).
...
### Spherical Projections (Transverse Mercator)
...
### Ellipsoidal Projections (Transverse Mercator)
...
- `tm_to_geographic(double a, double e2, double k0, double lon_mer, double FN, double FE, double N, double E, double* lat_rad, double* lon_rad)`: Converts TM to geographic on the ellipsoid using the Karney/Kawase expansions. Parameters as in `geographic_to_tm` with `N` and `E` as inputs, and `lat_rad` and `lon_rad` as outputs.

### Ellipsoidal Projections (Polar stereographic)
...
### Ellipsoidal Projections (obsoleted DMATM Transverse Mercator expansions)
